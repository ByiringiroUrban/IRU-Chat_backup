...
const SmartSummariesPage: React.FC = () => {
      ...
      <main className={`flex-1 overflow-y-auto p-6 ${isDark ? 'bg-[#0b0f17]' : 'bg-[#f6f8fd]'}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="space-y-1">
            <h1 className={`text-2xl font-semibold ${textPrimary}`}>Smart Summaries</h1>
            <p className={`text-sm ${textMuted}`}>
              AI-powered summaries of conversations, threads, and chat history.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <section className={`${cardBg} border ${cardBorder} rounded-[22px] ${shadow} overflow-hidden`}>
              <div className={`px-4 py-3.5 flex items-center justify-between gap-2.5 border-b ${cardBorder} ${cardHeaderBg}`}>
                <h2 className={`text-sm font-medium tracking-wide m-0 ${textPrimary}`}>Generate Summary</h2>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="space-y-2">
                  <label className={`block text-xs ${textMuted}`}>Time period</label>
                  <select className={`w-full px-3 py-2 rounded-lg ${cardHeaderBg} border ${cardBorder} ${textPrimary} text-sm`}>
                    <option>Today</option>
                ...
                <div className="space-y-2">
                  <label className={`block text-xs ${textMuted}`}>Summary type</label>
                  <select className={`w-full px-3 py-2 rounded-lg ${cardHeaderBg} border ${cardBorder} ${textPrimary} text-sm`}>
                    <option>Brief</option>
                ...
                <p className={`text-xs ${textMuted}`}>
                  Wireframe: AI summarization with customizable templates.
                </p>
              </div>
            </section>
            ...
            <section className={`${cardBg} border ${cardBorder} rounded-[22px] ${shadow} overflow-hidden`}>
              <div className={`px-4 py-3.5 border-b ${cardBorder} ${cardHeaderBg}`}>
                <h2 className={`text-sm font-medium tracking-wide m-0 ${textPrimary}`}>Summary History</h2>
              </div>
              <div className="p-4 space-y-4">
                <table className="w-full border-collapse text-[13px]">
                  ...
                  <tbody>
                    {summaries.map((summary, index) => (
                      <tr key={index}>
                        <td className={`border-b ${tableBorder} px-2 py-2.5 ${textPrimary}`}>{summary.chat}</td>
                        <td className={`border-b ${tableBorder} px-2 py-2.5 ${textPrimary}`}>{summary.period}</td>
                        <td className={`border-b ${tableBorder} px-2 py-2.5 ${textPrimary}`}>{summary.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export default SmartSummariesPage;
