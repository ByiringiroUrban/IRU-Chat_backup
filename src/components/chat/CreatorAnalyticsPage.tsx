...
const CreatorAnalyticsPage: React.FC = () => {
  ...
  const metrics = [
    ...
    { metric: 'Engagement Rate', value: '—', change: '—' },
    { metric: 'Subscribers', value: '—', change: '—' },
  ];
      ...
      <main className={`flex-1 overflow-y-auto p-6 ${isDark ? 'bg-[#0b0f17]' : 'bg-[#f6f8fd]'}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="space-y-1">
            <h1 className={`text-2xl font-semibold ${textPrimary}`}>Creator Analytics</h1>
            <p className={`text-sm ${textMuted}`}>
              Track your performance, audience insights, and content metrics.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <section className={`${cardBg} border ${cardBorder} rounded-[22px] ${shadow} overflow-hidden`}>
              <div className={`px-4 py-3.5 flex items-center justify-between gap-2.5 border-b ${cardBorder} ${cardHeaderBg}`}>
                <h2 className={`text-sm font-medium tracking-wide m-0 ${textPrimary}`}>Key Metrics</h2>
                ...
                <table className="w-full border-collapse text-[13px]">
                  ...
                  <tbody>
                    {metrics.map((item, index) => (
                      <tr key={index}>
                        <td className={`border-b ${tableBorder} px-2 py-2.5 ${textPrimary}`}>{item.metric}</td>
                        <td className={`border-b ${tableBorder} px-2 py-2.5 ${textPrimary}`}>{item.value}</td>
                        <td className={`border-b ${tableBorder} px-2 py-2.5 ${textPrimary}`}>{item.change}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <p className={`text-xs ${textMuted}`}>
                  Wireframe: connect to analytics API with charts and graphs.
                </p>
              </div>
            </section>
            ...
            <section className={`${cardBg} border ${cardBorder} rounded-[22px] ${shadow} overflow-hidden`}>
              <div className={`px-4 py-3.5 border-b ${cardBorder} ${cardHeaderBg}`}>
                <h2 className={`text-sm font-medium tracking-wide m-0 ${textPrimary}`}>Audience Insights</h2>
              </div>
              <div className="p-4 space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className={`text-xs ${textMuted}`}>Demographics</span>
                    <button className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${buttonInactive}`}>
                      View
                    </button>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`text-xs ${textMuted}`}>Top Content</span>
                    <button className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${buttonInactive}`}>
                      View
                    </button>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`text-xs ${textMuted}`}>Engagement Trends</span>
                    <button className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${buttonInactive}`}>
                      View
                    </button>
                  </div>
                ...
                <div className={`p-3 rounded-lg border ${cardBorder} ${cardHeaderBg}`}>
                  <p className={`text-xs ${textMuted}`}>
                    Detailed analytics and insights help you understand your audience and optimize your content strategy.
                  </p>
                </div>
                <p className={`text-xs ${textMuted}`}>
                  Access detailed reports and export data for analysis.
                </p>
              </div>
            </section>
          </div>
