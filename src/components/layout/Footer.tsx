  ...
  return (
    <footer className="bg-gradient-to-r from-techdark to-techblue-900 text-white relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-15 pointer-events-none">
        <div className="absolute w-40 h-40 bg-techblue-400 rounded-full -top-10 -left-10 animate-pulse" style={{ animationDuration: '6s' }}></div>
        <div className="absolute w-60 h-60 bg-techblue-500 rounded-full top-40 right-20 animate-pulse" style={{ animationDuration: '8s' }}></div>
        <div className="absolute w-20 h-20 bg-techpurple-light rounded-full bottom-10 left-1/4 animate-pulse" style={{ animationDuration: '4s' }}></div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          <div className="transform transition-transform duration-500 hover:-translate-y-2">
            ...
            <p className="text-gray-300 mb-6">
              Transforming industries with cutting-edge AI solutions, empowering businesses to achieve more.
            </p>
            <div className="flex space-x-4">
              <a href="https://www.youtube.com/@irutv-2060" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-techblue hover:scale-110 transition-all duration-300">
                <Youtube size={20} />
              </a>
              <a href="https://www.instagram.com/techriserwanda/" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-techblue hover:scale-110 transition-all duration-300">
                <Twitter size={20} />
              </a>
              <a href="https://www.linkedin.com/in/iru-business-group-571ba3334?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-techblue hover:scale-110 transition-all duration-300">
                <Linkedin size={20} />
              </a>
              <a href="https://x.com/IRUBUSINESSES?t=QHreTJ4D1GtZfix4tQIpyw&s=09" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-techblue hover:scale-110 transition-all duration-300">
                <Youtube size={20} />
              </a>
              <a href="https://www.instagram.com/irubusinessgroup?igsh=Y2s1N25qY2xzM2Zu" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-techblue hover:scale-110 transition-all duration-300">
                <Instagram size={20} />
              </a>
            </div>
          </div>
          ...
          <div className="transform transition-transform duration-500 hover:-translate-y-2">
            ...
            <ul className="space-y-2">
              {["About Us", "Program", "Gallery", "Contact"].map((item, index) => (
                <li key={index} className="transform hover:translate-x-2 transition-transform duration-300">
                  <Link
                    to={`/about`}
                    className="text-gray-300 hover:text-white flex items-center"
                  >
                    <span className="w-0 h-0.5 bg-techblue mr-0 transition-all duration-300"></span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex space-x-6 mt-4 md:mt-0">
              {["Privacy Policy", "Terms of Service", "Sitemap"].map((item, index) => (
                <Link key={index} to={`/${item.toLowerCase().replace(/\s+/g, '-')}`} className="text-gray-400 text-sm hover:text-white hover:underline transition-all duration-300">
                  {item}
                </Link>
              ))}
            </div>
